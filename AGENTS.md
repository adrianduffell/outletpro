...
* Guards: Use one condition per guard clause. Instead of combining multiple conditions with `&&` or `||`, write each guard as separate `if` statements to improve readability and debugging.
