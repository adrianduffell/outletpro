...
* Each test is self-contained. Never use shared `setUp` / `tearDown` or similar variants in PHPUnit, nor `test.beforeEach` / `test.afterEach` and it's similar variants in Playwright. Each test is responsible for arranging the environment to be the exact state it needs.
