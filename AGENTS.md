...
* Don't use `ob_start` in tests. Use tPHPUnit's `expect*` methods and the Arrange-Expect-Act pattern. Use `expectException()` for exceptions and `expectOutput` for echo/printed output.
