...
class Test_Register_Clearance_Badge_Experimental_Scale_Setting extends WP_UnitTestCase {
	...
	public function test_registers_clearance_badge_experimental_scale_setting(): void {
		// Arrange.
		unregister_setting( 'wc_clearance', CLEARANCE_BADGE_EXPERIMENTAL_SCALE_OPTION );
		...
		$this->assertArrayHasKey( CLEARANCE_BADGE_EXPERIMENTAL_SCALE_OPTION, $settings );
	}

	public function test_setting_type_is_number(): void {
		// Arrange.
		unregister_setting( 'wc_clearance', CLEARANCE_BADGE_EXPERIMENTAL_SCALE_OPTION );
		...
		// Assert.
		$settings = get_registered_settings();
		$this->assertSame( 'number', $settings[ CLEARANCE_BADGE_EXPERIMENTAL_SCALE_OPTION ]['type'] );
	}
	...
	public function test_setting_is_shown_in_rest(): void {
		...
		// Assert.
		$this->assertArrayHasKey( CLEARANCE_BADGE_EXPERIMENTAL_SCALE_OPTION, $response->get_data() );
	}
	...
	public function test_setting_can_be_updated_via_rest_with_valid_value(): void {
		...
		$request->set_param( CLEARANCE_BADGE_EXPERIMENTAL_SCALE_OPTION, 1.4 );
		$response = rest_do_request( $request );
		$data     = $response->get_data();

		// Assert.
		$this->assertSame( 1.4, $data[ CLEARANCE_BADGE_EXPERIMENTAL_SCALE_OPTION ] );
	...
	public function test_setting_rest_schema_minimum_is_zero(): void {
		...
		// Act.
		$request = new WP_REST_Request( 'POST', '/wp/v2/settings' );
		$request->set_param( CLEARANCE_BADGE_EXPERIMENTAL_SCALE_OPTION, -1 );
		$response = rest_do_request( $request );
