...
class Test_Render_Clearance_Badge_Callback extends WP_UnitTestCase {
	...
	public function test_badge_uses_global_bg_color_setting(): void {
		// Arrange.
		deinit_blocks();
		register_clearance_badge_block();
		register_clearance_status_taxonomy();
	...
	public function test_badge_uses_global_text_color_setting(): void {
		// Arrange.
		deinit_blocks();
		register_clearance_badge_block();
		register_clearance_status_taxonomy();
		seed_clearance_status_taxonomy();
		update_option( CLEARANCE_BADGE_TEXT_COLOR_OPTION, '#0000FF' );
		$product = \WC_Helper_Product::create_simple_product();
		add_to_clearance( $product );
		$block = new WP_Block(
			array(
