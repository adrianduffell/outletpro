...
class Test_Register_Clearance_Badge_Density_Setting extends WP_UnitTestCase {
	...
	public function test_setting_can_be_updated_via_rest_with_valid_value(): void {
		...
		$request->set_param( CLEARANCE_BADGE_DENSITY_OPTION, 60 );
		$response = rest_do_request( $request );
		$data     = $response->get_data();

		// Assert.
		$this->assertSame( 60, $data[ CLEARANCE_BADGE_DENSITY_OPTION ] );
	...
	public function test_setting_rest_schema_maximum_is_one_hundred(): void {
		...
		// Act.
		$request = new WP_REST_Request( 'POST', '/wp/v2/settings' );
		$request->set_param( CLEARANCE_BADGE_DENSITY_OPTION, 101 );
		$response = rest_do_request( $request );
